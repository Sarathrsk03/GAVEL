...
def train_model():
    # Check if model already exists to avoid re-training
    if os.path.exists("./results/forgery_model.pth"):
        print("\n[INFO] Trained model found at ./results/forgery_model.pth. Skipping training.")
        return
    ...
    # Save helper files (config, tokenizer) via save_pretrained
    model.save_pretrained("./results")
    tokenizer.save_pretrained("./results")
    
    # Save the actual weights as .pth as requested
    torch.save(model.state_dict(), "./results/forgery_model.pth")
    print("[SUCCESS] Model saved to ./results/forgery_model.pth\n")


# ------------------------------------------------------------
# Prediction
# ------------------------------------------------------------
def predict(file_path="/Users/sarathrajan/Desktop/hackathon/agents/forgery_evidence_checker/dataset/test.txt"):
    print("\n================ PREDICTION STARTED ================\n")
    print(f"[INFO] Input file: {file_path}")

    model_path = "./results/forgery_model.pth"
    if not os.path.exists(model_path):
        print("[ERROR] Model file not found at ./results/forgery_model.pth. Please train the model first.")
        return

    print("[STEP] Loading tokenizer and model...")
    # Load from the saved directory
    try:
        # Load tokenizer from the directory (it was saved via save_pretrained)
        tokenizer = DistilBertTokenizer.from_pretrained("./results")
        
        # Initialize model structure then load weights
